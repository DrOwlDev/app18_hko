...
- Forecast chart: tall vertically; yellow forecast from HKT 00:00 through next-day 00:00 (backfill past hours HKO drops on refresh); observed as line + single current black marker (no per-point dots); Y-axis every whole °C (clickable → purple [T, T+0.99] bucket band + remaining-forecast above ≥T / below <T counts with per-degree breakdown); red "now" line labeled with latest HK Observatory air temp from text readings / 1-min CSV; extra horizontal lines when forecasted min/max ≠ observed.
