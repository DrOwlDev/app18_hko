...
- Forecast tab: Hide Table / Hide Bucket on by default (filters under chart); HKT day as `ddd D/M` with `(F)` for forecast-only; Forecast Model selector ~70% width labeled `ddd D/M HH:mm (ddd D/M Ham)` (refresh first); orange fill when selected model refresh is older than 3 hours; Refresh Data above chart (Windows: re-collect HKO; Pages: open GitHub Actions).
- Forecast chart: tall vertically; green forecast from HKT 00:00 through next-day 00:00 (backfill past hours HKO drops on refresh); observed as line + single current black marker (no per-point dots); hourly HKO OCF weather icons above chart (rain/shower/thunder red; rain fills neighboring hours until next explicit icon; rain hours use red X on the green line); grey vertical grid aligned to hour ticks; Y-axis every whole °C (clickable → purple [T, T+0.99] bucket band; two-line probe with ABOVE exclusive of clicked bucket starting at T+1: `NC probe - ABOVE (N+1)C = … [ XC = n ; … ] · wv …` then `NC probe - BELOW NC = … · wv …` where wv is mean distance of those points from the ABOVE/BELOW boundary); red "now" line labeled with latest HK Observatory air temp from text readings / 1-min CSV; extra horizontal lines when forecasted min/max ≠ observed; below chart show latest HKO HQ webcam stills (west HK2 + east HKO).
