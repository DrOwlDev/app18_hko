...
- Forecast tab: Hide Table / Hide Bucket on by default (filters under chart); HKT day as `ddd D/M` with `(F)` for forecast-only; Forecast Model selector ~70% width labeled `ddd D/M HH:mm (ddd D/M Ham)` (refresh first); orange fill when selected model refresh is older than 3 hours; Refresh Data and Rain Forecast above chart (Windows Refresh: re-collect HKO; Pages Refresh: open GitHub Actions; Rain Forecast opens HKO OCF ncrf map).
